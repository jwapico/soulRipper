...
class SpotifySynchronizer():
    def __init__(self, sql_session: AsyncSession, spotify_client: SpotifyClient):
        self._sql_session = sql_session
        self._spotify_client = spotify_client

    async def update_db_with_all_playlists(self):
        ...
        if all_playlists_metadata:
            for playlist_metadata in all_playlists_metadata:
                await self.update_db_with_spotify_playlist(playlist_metadata)

    async def update_db_with_spotify_playlist(self, spotify_playlist_metadata: dict) -> Playlists:
    ...
    async def update_db_with_spotify_liked_tracks(self) -> Optional[Playlists]:
        ...
        relevant_tracks_data: List[Tuple[TrackData, datetime.datetime]] = self.get_track_data_from_playlist(liked_tracks_data)

        # create the playlist row
        liked_playlist_row = await PlaylistsRepository.add_playlist(self._sql_session, spotify_id=None, name="SPOTIFY_LIKED_SONGS", description="User liked songs on Spotify - This playlist is generated by SoulRipper")

        # add each track in the users liked songs to the database if it doesn't already exist
        await PlaylistsRepository.add_tracks_to_playlist(self._sql_session, relevant_tracks_data, liked_playlist_row)

        # commit and return the row
        await self._sql_session.commit()
        return liked_playlist_row

    def get_track_data_from_playlist(self, tracks: List[Dict]) -> List[Tuple[TrackData, datetime.datetime]]:
        relevant_data = []
        for track in tracks:
