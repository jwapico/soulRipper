...
def download_liked_songs(slskd_client: SoulseekDownloader, spotify_client: SpotifyClient, sql_session: Session, output_path: str, youtube_only: bool) -> bool:
    ...
    liked_playlist_tracks_rows = PlaylistsRepository.get_playlist_track_rows(sql_session, liked_playlist.id)
    ...
    try:
        ...
        for playlist_track_row in liked_playlist_tracks_rows:
            ...
            track_row = TracksRepository.get_track_from_id(sql_session=sql_session, track_id=track_id)

            if track_row is not None:
                if track_row.filepath is None:
                    artist_rows = ArtistsRepository.get_artists_for_track_id(sql_session, track_id)
...
def download_liked_tracks_from_spotify_data(slskd_client: SoulseekDownloader, spotify_client: SpotifyClient, sql_session, output_path: str):
    ...
    for track, _ in relevant_tracks_data:
        existing_track = TracksRepository.get_existing_track(sql_session, track)
        if existing_track is None:
            filepath = download_track(slskd_client, track, output_path)
            track.filepath = filepath

            track_row = TracksRepository.add_track(sql_session, track)
            track_rows_and_data.append((track_row, track))

    PlaylistsRepository.add_playlist(sql_session, spotify_id=None, name="SPOTIFY_LIKED_SONGS", description="User liked songs on Spotify - This playlist is generated by SoulRipper")

def download_playlist_from_spotify_url(slskd_client: SoulseekDownloader, spotify_client: SpotifyClient, sql_session: Session, playlist_url: str, output_path: str):
    """
    ...
    if playlist_info:
        ...
        for track_data, _ in relevant_tracks_data:
            existing_track_row = TracksRepository.get_existing_track(sql_session, track_data)
            ...
            if existing_track_row is None:
                ...
                new_track_row = TracksRepository.add_track(sql_session, track_data)
        ...
        PlaylistsRepository.add_playlist(sql_session, playlist_id, playlist_info["name"], playlist_info["description"])
        sql_session.commit()
