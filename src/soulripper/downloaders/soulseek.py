...
class SoulseekDownloader:
    ...
    # TODO: this function long ash prolly needs a refactor
        ...
        try:
            ...
            if self._client:
                ...
                while percent_complete < 100:
                    ...
                    slskd_download = await self._client.transfers.get_download(download_user, download_file_id)
    ...
    async def start_download(self, search_results, max_retries) -> Tuple[Optional[str], Optional[str], Optional[str]]:
        ...
        if self._client:
            for attempt_count, (file_data, file_user) in enumerate(search_results):
                ...
                try:
                    await self._client.transfers.enqueue(file_user, [file_data])
                except Exception as e:
    ...
    async def search(self, search_query: str) -> Optional[List]:
        ...
        if self._client:
            search = await self._client.searches.search_text(search_query)
            search_id = search["id"]
            ...
            while is_complete == False:
                search_state = await self._client.searches.state(search_id)
            ...
            search_results = await self._client.searches.search_responses(search_id)
    ...
    async def _search_file_id_from_filename(self, filename):
        ...
        if self._client:
            for download in await self._client.transfers.get_all_downloads():
